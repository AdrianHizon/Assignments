...
public class A5HizonAdrian {
    ...
    public static void main(String[] args) throws NoSuchAlgorithmException {
        ...
        A5HizonAdrian bitcoin = new A5HizonAdrian();
    ...
    private int validate(MessageDigest digest) throws NoSuchAlgorithmException {
        ...
        String currBuiltHash;
        int height = -1;
        
        for(int i = 1; i < blocks.size(); i++) {
        	prev = blocks.get(i - 1);
        	curr = blocks.get(i);
	        txArray = arrayListToArray(curr.getTransactions());
	        prevBlockHash = prev.getBlockHash();
	        currTopHash = tree.buildFrom(txArray);
	        currNonce = curr.getNonce();
	        currBlockHash = curr.getBlockHash();
	        currBuiltHash = UtilityFunctions.getSHA256(digest, prevBlockHash + currTopHash + currNonce);
	        System.out.println(currBlockHash + " " + currNonce + " " + currBuiltHash);
	        if(!currBlockHash.equals(currBuiltHash)) {
	        	height = i;
	        }
        }
        if(height != -1) {
    ...
    private Transaction[] arrayListToArray(ArrayList<Transaction> txAList) {
    	Transaction[] txArray = new Transaction[txAList.size()];
    	for(int i = 0; i < txAList.size(); i++) {
    		txArray[i] = txAList.get(i);
    	}
    	return txArray;
    }
    ...
    /*
     * Report
     * Question 1: The depth of a merkle tree is equal the number of levels of hashing to find the top hash.
     * Question 2: To detect corruption at the transaction level you must make n comparisons to the merkle tree by comparing each transaction one by one.
     * Question 3: Since computing the block hash of a block requires the block hash of the previous block then corrupting any part would be detected.
     */
}
