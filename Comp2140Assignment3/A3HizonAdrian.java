...
public class A3HizonAdrian {
	...
	public static void main(String [] args) {
		...
		/* Report
		 * 2c. The difference between linked lists and array lists is that you can access any element in an array list without having to traverse through any nodes.
		 * 2d. store3 is a List while store4 is a LinkedList, LinkedList only methods can be called on store4 but not on store3.
		 * 2e. The advantage of using the LinkedList declaration in store4 is that you can use methods that are only usable on LinkedList. The disadvantage is that while store4 cannot be changed to any class equal or higher in Collection hierarchy
		 */
	}
}
